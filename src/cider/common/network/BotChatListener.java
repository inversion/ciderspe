...
public class BotChatListener implements ChatManagerListener {
    ...
    public void chatCreated(Chat chat, boolean createdLocally)
    {
    	...
    	if( !StringUtils.parseResource( chat.getParticipant() ).startsWith( Client.RESOURCE ) )
    		return;
    	
        if( chats.containsKey( StringUtils.parseName( chat.getParticipant() ) ) )
        {
            if( DEBUG )
                System.out.println( chat.getParticipant() + " initiated chat, but is already connected from another CIDER client, alerting new instance to quit..." );
            try {
    ...
    /**
     *  Called by message listeners when they receive a quit request from the client.
     * 	Remove the listener from the chat and remove its entry from the table.
     * 
     * @param user
     */
