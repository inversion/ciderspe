...
public class ClientMessageListener implements MessageListener {	
	...
	private Pattern fileMatch = Pattern.compile( "<file><path>(.+)</path><contents>(.+)</contents></file>" );
	private Matcher matcher = null;
	
	public ClientMessageListener( DirectoryViewComponent dirView, JTabbedPane tabbedPane ) {
	...
	public void processMessage(Chat chat, Message message) {
		...
		{
			try {
				dirView.constructTree( (CiderFileList) Base64.decodeToObject( body.substring( 9, body.length() ) ) );
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
