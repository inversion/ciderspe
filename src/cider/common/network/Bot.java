...
public class Bot
{
    ...
    private ConnectionConfiguration config = new ConnectionConfiguration( HOST, PORT, SERVICE_NAME );
    ...
    public Bot()
    {
        try
        {
            ...
            chatroom = new MultiUserChat( connection, CHATROOM_NAME + "@conference." + SERVICE_NAME );
    ...
    // Connect to the server as a reserved user to check if the bot is already online from another location
    private void checkForBot() throws XMPPException
    {
        XMPPConnection conn = new XMPPConnection( config );
        conn.connect();
    	conn.login( "ciderchecker", "checkerpw" );
    	chatroom = new MultiUserChat( conn, CHATROOM_NAME + "@conference." + SERVICE_NAME );
    	try {
			chatroom.create( "ciderchecker" );
		} catch (XMPPException e) {
			System.err.println( "Error: Chatroom already exists, this means the bot is already online, or someone else has created the room.");
