...
public class TypingEvent
{
    ...
    public TreeMap<Double, TypingEvent> explode(final double keyRange)
    {
        TreeMap<Double, TypingEvent> result = new TreeMap<Double, TypingEvent>();
        final double inc = keyRange / this.text.length();
        int pos = 0;
        double t;
        for (char c : this.text.toCharArray())
        {
            pos++;
            t = this.time + inc * pos;
            result.put(t, new TypingEvent(this.time, this.mode, pos, "" + c));
        }
        return result;
    }
